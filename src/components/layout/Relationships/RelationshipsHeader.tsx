...
const RelationshipsHeader: React.FC<RelationshipsHeaderProps> = ({
  activeTab,
  onTabChange,
  onOpenLauncher,
}) => {
  ...
  shadow-sm"
        ...
        <div className="flex items-center gap-2">
          {/* Help */}
          <Tooltip title="Add relationships">
            <button
              onClick={onCreateNew}
              className="
      w-9 h-9
      ...
      <RelationshipsForm
        ...
        onSave={async (formData) => {
          try {
            ...
            if (!accountId) {
              toast.error("Active account not found");
              return;
            }
            ...
            const res = await fetch(
              `${baseUrl()}/api/Counterparty/${accountId}/Counterparty`,
              {
                method: "POST",
                headers: {
            ...
            if (!res.ok) {
              ...
              throw new Error(err || "Failed to create counterparty");
            }
