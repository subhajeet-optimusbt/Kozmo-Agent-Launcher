...
const LauncherModal: React.FC<LauncherModalProps> = ({
  ...
  const [searchText, setSearchText] = useState("");
  const [focusedIndex, setFocusedIndex] = useState(0);
  const [, setPinnedKeys] = useLocalStorage<string[]>(
    "launcher_pinned",
    [],
  );
...
const activeAppKey = useMemo(() => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [launcherOpen, leftApps, focusedIndex]);

  const scrollIntoView = (index: number) => {
    const list = containerRef.current?.querySelectorAll("[data-app]");
    if (!list || index < 0 || index >= list.length) return;
    (list[index] as HTMLElement).scrollIntoView({
      block: "nearest",
      behavior: "smooth",
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="p-3 bg-white/80 backdrop-blur-xl border-b border-gray-200/50">
          <div className="flex items-center gap-4 px-5 py-2 bg-white rounded-2xl border border-gray-200/60">
            <SearchOutlined className="text-2xl text-emerald-500" />
            <input
              data-launcher-search
              value={searchText}
              onChange={(e) => {
        ...
        <div className="flex-1 p-7">
          {leftApps.length === 0 ? (
            <div className="flex items-center justify-center h-full border-2 border-dashed rounded-2xl">
                ...
                {leftApps.map((app) => (
                  ...
                  >
                    <AppTile
                      ...
                      // isPinned={pinnedKeys.includes(app.key)}
                      // isFocused={focusedIndex === idx}
                         isFocused={app.key === activeAppKey}
                      onClick={() => openApp(app.key)}
                      onPinToggle={togglePin}
                    />
                  </div>
