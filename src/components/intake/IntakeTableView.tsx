...
export default function IntakeTableView({
  data,
  onSelect,
  sorter,
  onSortChange,
  ...
  data: any;
  onSelect: (c: Intake) => void;
  sorter: Sorter;
  onSortChange: (s: Sorter) => void;
}) {
  /* ---------------- SORT TOGGLE LOGIC ---------------- */
  const handleSortClick = (field: keyof Intake) => {
  ...
  const columns: ColumnsType<Intake> = [
    ...
    {
      title: <SortableHeader label="No of Docs" field="noOfDocuments" />,
      dataIndex: "noOfDocuments",
      key: "noOfDocuments",
      render: (v: string) => (
        <span
          className={v === "N/A" ? "text-gray-400 italic" : "text-gray-700"}
        >
          {v}
        </span>
      ),
